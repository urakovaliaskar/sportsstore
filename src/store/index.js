...
export default new Vuex.Store({
  ...
  getters: {
    ...
    productById: state => id =>
      state.pages[state.currentPage].find(p => p.id == id)
  },
  mutations: {
    ...
    clearPages(state) {
      state.pages.slice(0, state.pages.length);
    },
  ...
  actions: {
    ...
    async getPage(context, getPageCount) {
      let url =
        `${productsUrl}?_page=${context.state.currentPage}` +
        `&_limit=${context.state.pageSize}`;
    ...
    async addProduct(context, product) {
      let data = (await context.getters.authenticatedAxios(productsUrl, product))
        .data;
